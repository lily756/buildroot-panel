...
void aicwf_bus_deinit(struct device *dev)
{
	...
	/*
	 * The boot firmware disconnects and exposes the runtime USB function
	 * almost immediately. Sleeping here runs in usb_disconnect() context and
	 * blocks khubd long enough for that runtime connection to time out before
	 * it can be enumerated on a slow ARM9 host. Killing the anchored URBs below
	 * provides the teardown synchronization; the 500 ms busy wait did not.
	 */
    bus_if = dev_get_drvdata(dev);
